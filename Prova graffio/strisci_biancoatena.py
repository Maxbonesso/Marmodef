...
import numpy as np
original_path = 'biancoatena/biancoatena_patch/patchii2/normal/'
destination_path = 'biancoatena/biancoatena_patch/patchii1/abnormal/filetestdaeliminare'
...
for f in all_the_files_jpg:
    ...
    gaussian = cv2.GaussianBlur(p, (333, 333), 0)
    gaussian = 255 - (p - gaussian)
    kernel = np.ones((5, 5), np.uint8)
    gaussian = cv2.morphologyEx(gaussian, cv2.MORPH_OPEN, kernel)
    ret2,gaussian = cv2.threshold(gaussian,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    ...
    lines = cv2.HoughLinesP(gaussian, 1, np.pi / 360, 100, minLineLength=10, maxLineGap=10)
...
for f in all_the_files_png:
    ...
    p = cv2.imread(str(f), 0)
    out = cv2.cvtColor(p, cv2.COLOR_GRAY2BGR)
    gaussian = cv2.GaussianBlur(p, (111, 111), 0)
    gaussian = gaussian - p + 255
    kernel = np.ones((3, 3), np.uint8)
    gaussian = cv2.morphologyEx(gaussian, cv2.MORPH_CLOSE, kernel)
    ret2, gaussian = cv2.threshold(gaussian, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
